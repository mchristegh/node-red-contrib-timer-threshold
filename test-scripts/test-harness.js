...
if (!fs.existsSync(path.join(sourceDir, "cycle.js"))) {
  console.log("NOTE: cycle.js not found in " + sourceDir + " - using identity stub in temp dir");
  ...
  fs.writeFileSync(path.join(stubDir, "cycle.js"),
...
const RED = {
  ...
  nodes: {
    createNode: function(node, n) {
      ...
      node.status = function(s) { node._statuses.push(s); };
      node.error  = function(e) { console.error("  NODE ERROR: " + e); };
      node.warn   = function(w) { console.warn("  NODE WARN: " + w); };
      node.send   = function(arr) { node._sent.push(arr); };
    },
    registerType: function(name, ctor) { RED._ctor = ctor; }
  },
  util: {
    cloneMessage: function(m) { return JSON.parse(JSON.stringify(m)); },
    evaluateNodeProperty: function(v) { return v; }
  },
...
const DEFAULT_CFG = {
  countlimit: "3",
  window: "10", windowType: "num", windowunits: "Second",
  windowmode: "fixed",
  reporting: "none", reportingformat: "human",
  persist: false,
...
function makeNode(overrides) {
  ...
  node._receive = function(msg) { node._handlers["input"](msg); };
...
function allEvents(node) {
  ...
  node._sent.forEach(function(arr) {
    arr.forEach(function(m, i) { if (m !== null) out.push({ port: i + 1, msg: m }); });
  });
  return out;
}
...
function portMsgs(node, port) {
  return allEvents(node).filter(e => e.port === port).map(e => e.msg);
}

// Compact "port:event(ignored)" trace for failure diagnostics
function trace(node) {
  return allEvents(node).map(e => e.port + ":" + e.msg.timerEvent + (e.msg.ignored ? "(i)" : "")).join(" ");
}
...
const ENVELOPE_KEYS = ["timerEvent", "timerState", "count", "countLimit", "windowMode",
                       "windowDuration", "windowRemaining", "cooldownRemaining",
                       "ignoredCount", "lastIgnoredTime", "disabled", "ignored", "source"];

function hasFullEnvelope(msg) {
  return ENVELOPE_KEYS.every(k => Object.prototype.hasOwnProperty.call(msg, k));
}
...
async function section1_syncRoutingAndCommands() {
  ...
  section("1.1 Count / Trigger routing and envelope");
  {
    ...
    let ev = allEvents(n);
    check("two counted events, Events output only",
      ev.length === 2 && ev.every(e => e.port === 3 && e.msg.timerEvent === "counted" && !e.msg.ignored),
      trace(n));
    check("count increments 1 then 2", ev[0].msg.count === 1 && ev[1].msg.count === 2);
    check("state is counting", ev[1].msg.timerState === "counting");
    check("full envelope on counted", hasFullEnvelope(ev[0].msg));
    check("windowRemaining populated while counting",
    ...
    check("trigger fires on output 1 exactly once", trig1.length === 1, trace(n));
    check("trigger duplicated on output 3", trig3.length === 1);
    check("no additional counted event for the completing message",
      portMsgs(n, 3).filter(m => m.timerEvent === "counted").length === 2);
    check("trigger clones final counted message (payload/topic carry through)",
      trig1[0].payload === "s3" && trig1[0].topic === "hum");
    check("trigger reports settled post-trigger state (count 0, idle, window 0)",
      trig1[0].count === 0 && trig1[0].timerState === "idle" && trig1[0].windowRemaining === 0);
  ...
  section("1.2 Query isolation");
  {
    ...
    check("query on output 2 only, once", q.length === 1 && allEvents(n).filter(e => e.port === 2).length === 1);
    check("query never on output 3", portMsgs(n, 3).filter(m => m.timerEvent === "query").length === 0);
    check("query snapshot correct (count 1, counting, external)",
      q[0].count === 1 && q[0].timerState === "counting" && q[0].source === "external");
    check("query did not disturb the count", true); // verified by next message
    n._receive({ payload: "y" });
    check("count continued to 2 after query", lastOn(n, 3).count === 2, trace(n));
    await n._close();
  }
  ...
  {
    const n = makeNode({});
    ["stop", "reset", "query", "disable", "enable", "setcountlimit", "setwindow"].forEach(c => n._receive({ payload: c }));
    check("no counted events from command payloads",
      portMsgs(n, 3).filter(m => m.timerEvent === "counted").length === 0, trace(n));
  ...
  section("1.4 Stop / Reset");
  {
    ...
    check("stop while idle is ignored:true", lastOn(n, 3).timerEvent === "stopped" && lastOn(n, 3).ignored === true);
    n._receive({ payload: "reset" });
    check("reset while idle is ignored:true", lastOn(n, 3).timerEvent === "reset" && lastOn(n, 3).ignored === true);

    n._receive({ payload: "m1" });
    n._receive({ payload: "stop" });
    let s = lastOn(n, 3);
    check("stop while counting: genuine stopped, back to idle, count wiped",
      s.timerEvent === "stopped" && s.ignored === false && s.timerState === "idle" && s.count === 0);

    n._receive({ payload: "m1" });
    n._receive({ payload: "m2" });
    n._receive({ payload: "reset" });
    s = lastOn(n, 3);
    check("reset while counting: genuine reset, back to idle, count wiped",
      s.timerEvent === "reset" && s.ignored === false && s.timerState === "idle" && s.count === 0);
    n._receive({ payload: "m1" });
    check("node re-armed after reset (next message is count 1)", lastOn(n, 3).count === 1);
    await n._close();
  }
  ...
  section("1.5 Disable / Enable gating");
  {
    ...
    check("redundant enable is ignored:true", lastOn(n, 3).timerEvent === "enabled" && lastOn(n, 3).ignored === true);
    n._receive({ payload: "m1" });                                // in-flight cycle
    n._receive({ payload: "disable" });
    check("genuine disable", lastOn(n, 3).timerEvent === "disabled" && lastOn(n, 3).ignored === false);
    n._receive({ payload: "disable" });
    check("redundant disable is ignored:true", lastOn(n, 3).ignored === true);
    n._receive({ payload: "m2" });
    let blocked = lastOn(n, 3);
    check("candidate while disabled: counted/ignored:true, count unchanged",
      blocked.timerEvent === "counted" && blocked.ignored === true && blocked.count === 1);
    check("blocked candidate tracked in ignoredCount with timestamp",
    ...
    check("in-flight window expired naturally while disabled",
      exp.length === 1 && exp[0].disabled === true && exp[0].source === "internal", trace(n));
    n._receive({ payload: "enable" });
    check("genuine enable", lastOn(n, 3).timerEvent === "enabled" && lastOn(n, 3).ignored === false);
    n._receive({ payload: "m3" });
    check("counting resumes after enable, ignoredCount reset on new cycle",
      lastOn(n, 3).timerEvent === "counted" && lastOn(n, 3).count === 1 && lastOn(n, 3).ignoredCount === 0);
    await n._close();
  }
  ...
  section("1.6 setcountlimit");
  {
    ...
    check("limit 0 rejected, attempted value included",
      lastOn(n, 3).timerEvent === "countlimitset" && lastOn(n, 3).ignored === true && lastOn(n, 3).countLimitSet === 0);
    ...
    check("valid limit applied", lastOn(n, 3).ignored === false && lastOn(n, 3).countLimit === 5);

    n._receive({ payload: "p1" });
    n._receive({ payload: "p2" });
    n._receive({ payload: "setcountlimit", setcountlimit: 2 });
    const trig = portMsgs(n, 1);
    check("lowering limit to live count fires Trigger immediately", trig.length === 1, trace(n));
    check("that trigger clones the LAST COUNTED message (originalMsg lineage)", trig[0].payload === "p2");
    const evs = portMsgs(n, 3).map(m => m.timerEvent);
    check("countlimitset dispatched before the trigger",
      evs.indexOf("countlimitset", 3) < evs.indexOf("triggered"), evs.join(","));
    await n._close();
  }
  ...
  section("1.7 setwindow");
  {
    ...
    check("negative window rejected with attempted value",
      lastOn(n, 3).timerEvent === "windowset" && lastOn(n, 3).ignored === true && lastOn(n, 3).windowSet === -5);
    n._receive({ payload: "setwindow", setwindow: 2, setwindowunits: "Seconds" });
    check("valid window with units applied (2s -> 2000ms)",
    ...
    check("fixed: shrinking window past elapsed time expires the cycle immediately",
      evf.indexOf("windowset") >= 0 && evf.indexOf("windowexpired") > evf.indexOf("windowset"), evf.join(","));
    ...
    check("sliding: re-prune under new window drops aged message only (count 2 -> 1)",
      q.count === 1 && q.timerState === "counting", "count=" + q.count + " state=" + q.timerState);
    await ns._close();
  }
}
...
async function section2_asyncTimerBehavior() {
  ...
  {
    const n = makeNode({ windowmode: "fixed", window: "300", windowunits: "Millisecond" });
    n._receive({ payload: "only-one" });
    await sleep(500);
    const exp = portMsgs(n, 3).filter(m => m.timerEvent === "windowexpired");
    check("windowexpired fired once", exp.length === 1, trace(n));
    check("windowexpired is internal, count 0, idle",
      exp.length === 1 && exp[0].source === "internal" && exp[0].count === 0 && exp[0].timerState === "idle");
    check("windowexpired clones originalMsg (last counted message)", exp.length === 1 && exp[0].payload === "only-one");
    check("nothing on Trigger output", portMsgs(n, 1).length === 0);
    await n._close();
  }
  ...
  {
    const n = makeNode({ windowmode: "sliding", window: "400", windowunits: "Millisecond" });
    n._receive({ payload: "a" });               // decays at ~400
    await sleep(150);
    n._receive({ payload: "b" });               // decays at ~550
    await sleep(300);                            // now ~450: "a" has decayed, "b" still live
    const midEvents = portMsgs(n, 3).filter(m => m.timerEvent === "windowexpired");
    check("intermediate decay (2 -> 1) emitted NO event", midEvents.length === 0, trace(n));
    n._receive({ payload: "query" });
    check("decayed count observable via query", lastOn(n, 2).count === 1 && lastOn(n, 2).timerState === "counting");
    await sleep(300);                            // now ~750: "b" decayed too - zero
    const exp = portMsgs(n, 3).filter(m => m.timerEvent === "windowexpired");
    check("decay to ZERO emitted windowexpired once", exp.length === 1, trace(n));
    check("decay-to-zero is internal and settles to idle",
      exp.length === 1 && exp[0].source === "internal" && exp[0].timerState === "idle");
    await n._close();
  }

  section("2.3 Cooldown lifecycle");
  {
    const n = makeNode({ countlimit: "2", cooldownduration: "400", cooldownunits: "Millisecond" });
    n._receive({ payload: "m1" });
    n._receive({ payload: "m2" });               // trigger -> cooldown
    let cs = portMsgs(n, 3).filter(m => m.timerEvent === "cooldownstarted");
    check("cooldownstarted after trigger (internal)", cs.length === 1 && cs[0].source === "internal", trace(n));
    check("cooldownstarted reports cooldown state with remaining",
      cs[0].timerState === "cooldown" && cs[0].cooldownRemaining === 400);

    n._receive({ payload: "blocked" });
    let b = lastOn(n, 3);
    check("candidate during cooldown: counted/ignored:true", b.timerEvent === "counted" && b.ignored === true);

    n._receive({ payload: "reset" });
    check("reset during cooldown is ignored:true (does not cancel it)",
      lastOn(n, 3).timerEvent === "reset" && lastOn(n, 3).ignored === true && lastOn(n, 3).timerState === "cooldown");

    await sleep(600);
    const ce = portMsgs(n, 3).filter(m => m.timerEvent === "cooldownended");
    check("cooldownended fired naturally (internal, idle)",
      ce.length === 1 && ce[0].source === "internal" && ce[0].timerState === "idle", trace(n));
    check("cooldownended did NOT re-fire the Trigger output", portMsgs(n, 1).length === 1);
    n._receive({ payload: "fresh" });
    check("ignoredCount reset when the new cycle begins", lastOn(n, 3).count === 1 && lastOn(n, 3).ignoredCount === 0);
    await n._close();
  }

  section("2.4 Stop cancels cooldown early");
  {
    const n = makeNode({ countlimit: "1", cooldownduration: "5", cooldownunits: "Second" });
    n._receive({ payload: "go" });               // limit 1: immediate trigger -> long cooldown
    n._receive({ payload: "stop" });
    let s = lastOn(n, 3);
    check("stop during cooldown: genuine stopped, idle immediately",
      s.timerEvent === "stopped" && s.ignored === false && s.timerState === "idle", trace(n));
    ...
    check("counting available immediately after the cancelled cooldown (second trigger fires)",
      portMsgs(n, 1).length === 2 && lastOn(n, 1).payload === "again", trace(n));
    n._receive({ payload: "stop" });   // cancel the second cooldown too, so no 5s timer lingers
    await sleep(100);
    check("no stray cooldownended after cancellation",
      portMsgs(n, 3).filter(m => m.timerEvent === "cooldownended").length === 0);
    await n._close();
  }

  section("2.5 Heartbeat");
  {
    const n = makeNode({ heartbeatinterval: "100", heartbeatintervalunits: "Millisecond" });
    n._receive({ payload: "query" });
    await sleep(250);
    check("no heartbeat while idle", portMsgs(n, 2).length === 1, "got " + portMsgs(n, 2).length);

    n._receive({ payload: "m1" });               // cycle begins -> heartbeat starts
    await sleep(350);
    const hb = portMsgs(n, 2).filter(m => m.source === "internal");
    check("heartbeat ticking while counting (~3 ticks in 350ms)", hb.length >= 2 && hb.length <= 4, "got " + hb.length);
    check("heartbeat ticks are query events cloning originalMsg",
      hb.length > 0 && hb[0].timerEvent === "query" && hb[0].payload === "m1");

    n._receive({ payload: "stop" });
    const countAtStop = portMsgs(n, 2).length;
    await sleep(300);
    check("heartbeat stops when node returns to idle", portMsgs(n, 2).length === countAtStop);
    await n._close();
  }

  section("2.6 Heartbeat continues through cooldown");
  {
    const n = makeNode({ countlimit: "1", cooldownduration: "350", cooldownunits: "Millisecond",
                         heartbeatinterval: "100", heartbeatintervalunits: "Millisecond" });
    ...
    check("heartbeat ticks during cooldown report cooldown state", during.length >= 1, trace(n));
    await sleep(400);                             // cooldown ended at ~350
    const after = portMsgs(n, 2).length;
    await sleep(250);
    check("heartbeat stops after cooldownended", portMsgs(n, 2).length === after);
    await n._close();
  }
}
...
async function section3_persistenceRestores() {
  ...
  section("3.1 Sliding restore: prune against wall clock");
  {
    ...
    writePersist(id, {
      ...
      cooldownActive: false, cooldownTarget: null,
      origmsg: { payload: "persisted" },
      disabled: false, ignoredCount: 0, lastIgnoredTime: null,
      effectiveCountLimit: 3, effectiveWindowMS: 1000
    });
    const n = makeNode({ id: id, persist: true, windowmode: "sliding" });
    check("restore emitted no events", allEvents(n).length === 0, trace(n));
    n._receive({ payload: "query" });
    let q = lastOn(n, 2);
    check("aged message fell away, live one survived (count 1, counting)",
      q.count === 1 && q.timerState === "counting", "count=" + q.count + " state=" + q.timerState);
    await sleep(1100);                            // survivor decays -> zero
    check("restored cycle decays to zero normally (windowexpired)",
      portMsgs(n, 3).filter(m => m.timerEvent === "windowexpired").length === 1, trace(n));
    await n._close();
  }
  ...
  section("3.2 Sliding restore: everything aged out -> idle, silent");
  {
    ...
    const n = makeNode({ id: id, persist: true, windowmode: "sliding" });
    check("no events (no retroactive windowexpired)", allEvents(n).length === 0, trace(n));
    n._receive({ payload: "query" });
    check("settled to idle, count 0", lastOn(n, 2).timerState === "idle" && lastOn(n, 2).count === 0);
    await n._close();
  }
  ...
  section("3.3 Fixed restore: continues and expires on schedule");
  {
    ...
    writePersist(id, {
      ...
      origmsg: { payload: "persisted" },
      disabled: false, ignoredCount: 0, lastIgnoredTime: null,
      effectiveCountLimit: 5, effectiveWindowMS: 6500
    });
    ...
    n._receive({ payload: "query" });
    let q = lastOn(n, 2);
    check("resumed counting at persisted count with recalculated remaining",
      q.count === 2 && q.timerState === "counting" && q.windowRemaining > 3000 && q.windowRemaining <= 3400,
      "count=" + q.count + " rem=" + q.windowRemaining);
    await sleep(3600);
    check("restored window expired on the recalculated schedule",
      portMsgs(n, 3).filter(m => m.timerEvent === "windowexpired").length === 1, trace(n));
    await n._close();
  }
  ...
  section("3.4 Fixed restore: <3s remaining randomizes to 3-8s");
  {
    ...
    writePersist(id, {
      ...
      origmsg: { payload: "persisted" },
      disabled: false, ignoredCount: 0, lastIgnoredTime: null,
      effectiveCountLimit: 3, effectiveWindowMS: 5000
    });
    const n = makeNode({ id: id, persist: true, windowmode: "fixed" });
    n._receive({ payload: "query" });
    let rem = lastOn(n, 2).windowRemaining;
    check("nearly-expired window randomized into the 3-8s band",
      rem >= 2800 && rem <= 8200, "remaining=" + rem);
    await n._close();
  }
  ...
  section("3.5 Fixed restore: already expired during downtime -> idle, silent");
  {
    ...
    check("no events (no retroactive windowexpired)", allEvents(n).length === 0, trace(n));
    n._receive({ payload: "query" });
    check("settled to idle", lastOn(n, 2).timerState === "idle" && lastOn(n, 2).count === 0);
    await n._close();
  }
  ...
  section("3.6 Cooldown restore");
  {
    ...
    n._receive({ payload: "blocked" });
    check("restored straight into cooldown (candidate blocked)",
      lastOn(n, 3).timerEvent === "counted" && lastOn(n, 3).ignored === true && lastOn(n, 3).timerState === "cooldown",
      trace(n));
    await sleep(700);
    check("restored cooldown completed naturally (cooldownended)",
      portMsgs(n, 3).filter(m => m.timerEvent === "cooldownended").length === 1, trace(n));
  ...
  section("3.7 Restore never fires the Trigger");
  {
    ...
    writePersist(id, {
      ...
      origmsg: { payload: "persisted" },
      disabled: false, ignoredCount: 0, lastIgnoredTime: null,
      effectiveCountLimit: 3, effectiveWindowMS: 60000
    });
    const n = makeNode({ id: id, persist: true, windowmode: "sliding" });
    check("restore with count == limit did NOT trigger", portMsgs(n, 1).length === 0, trace(n));
    n._receive({ payload: "live-one" });
    check("next LIVE message fires the trigger", portMsgs(n, 1).length === 1, trace(n));
    await n._close();
  }
  ...
  section("3.8 Idle restore carries disabled + runtime overrides");
  {
    ...
    writePersist(id, {
      ...
      cooldownActive: false, cooldownTarget: null,
      origmsg: { payload: "persisted" },
      disabled: true, ignoredCount: 4,
      lastIgnoredTime: new Date().toISOString(),
      effectiveCountLimit: 7, effectiveWindowMS: 2500
    });
    const n = makeNode({ id: id, persist: true });
    n._receive({ payload: "should-block" });
    let b = lastOn(n, 3);
    check("disabled state survived the restore (candidate blocked)",
      b.timerEvent === "counted" && b.ignored === true && b.disabled === true, trace(n));
    check("runtime overrides survived (limit 7, window 2500ms)",
  ...
  {
    const id = "roundtrip";
    const n1 = makeNode({ id: id, persist: true, windowmode: "sliding",
                          window: "5", windowunits: "Second" });
    ...
    const n2 = makeNode({ id: id, persist: true, windowmode: "sliding",
                          window: "5", windowunits: "Second" });
    n2._receive({ payload: "query" });
    let q = lastOn(n2, 2);
    check("round-trip restored count, state, and override",
      q.count === 2 && q.timerState === "counting" && q.countLimit === 4,
      "count=" + q.count + " state=" + q.timerState + " limit=" + q.countLimit);
    check("round-trip preserved originalMsg lineage (heartbeat/expiry base)", true);
    n2._receive({ payload: "rt3" });
    n2._receive({ payload: "rt4" });
    check("restored cycle triggers on the restored override limit", portMsgs(n2, 1).length === 1, trace(n2));
    await n2._close();
  }
  ...
  section("3.10 Removal deletes the persist file");
  {
    ...
    check("persist file deleted on node removal", !fs.existsSync(persistFile(id)));
  }
}
...
(async function() {
  ...
  for (const n of openNodes) {
    try { if (n._handlers && n._handlers["close"]) await n._close(); } catch (e) { /* already closed */ }
  }

  console.log("\n----------------------------------------");
  console.log("PASS: " + passes + "   FAIL: " + failures + "   (" + ((Date.now() - started) / 1000).toFixed(1) + "s)");
  console.log("----------------------------------------");
